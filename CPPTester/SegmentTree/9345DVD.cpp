...
int main(){
	...
	while (t--){
		...
		ffor(i,m){
			cin >> q >> a >> b;
			if(q==0){
				//cout << a<< " " <<b << " " <<"q-0: "<< root->rangeMin[a].fi << " " <<root->rangeMin[b].fi << endl;
				int aprev = root->query(a,a).fi;
				root->update(a,root->query(b,b).fi);
				root->update(b,aprev);
				
			}
			else{
				pii ans = root->query(a,b);
				if(ans.fi == a && ans.se == b){
					cout << "YES"<<endl;
				}
				else cout << "NO"<<endl;
			}
